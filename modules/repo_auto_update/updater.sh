#!/usr/bin/env bash
# Generic repository-controlled automatic updater.
...
for control in "$RELEASE_FILE" "$SEQUENCE_FILE"; do
...
if [[ "$installed" == "$target" ]]; then
    ...
    log "target ${target:0:12} is already deployed"
    exit 0
fi
...
if [[ -n "$installed" ]] && git -C "$checkout" cat-file -e "$installed^{commit}" \
        2>/dev/null && git -C "$checkout" merge-base --is-ancestor \
