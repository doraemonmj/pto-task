...
branch ancestry checks, locking, unprivileged verification, deployment, and
installed-version markers. It deliberately contains no tool-specific
installation logic.
...
3. A repository manifest with schema:
   ...
   ```json
   {"schema":1,"enabled":true,"target":"","activation":"automatic",
    "sequence":0,"allow_rollback":false}
   ```

Copy `updater.sh`, `manifest.py`, and the two systemd templates together. A
managed server may keep the timer enabled while the manifest has no target;
this polls normally without selecting or deploying a revision. The tool-specific
apply adapter owns safe installation and activation behavior.
...
Successful application writes `installed-revision` and `installed-sequence`
below the configured state directory only after the adapter has deployed and
handled activation of the target successfully.
